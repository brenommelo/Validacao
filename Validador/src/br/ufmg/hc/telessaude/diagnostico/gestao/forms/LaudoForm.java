...
//        }
    ...
    public static ConteudoECG12 processarConteudoExame(Exame exame) throws Exception {
        if (exame.getConteudosExames() != null && exame.getConteudosExames().size() > 0) {
            ...
            if (!conteudo.getTAXAAMOSTRAGEM().isEmpty()) {
                int taxaAmostragem = Integer.parseInt(retornaNumero(conteudo.getTAXAAMOSTRAGEM()));
                if(taxaAmostragem == 300){
                    taxaAmostragem =150;
                }
                conteudoEcg.setAmostragem(taxaAmostragem);
            }
...
//                break;
    ...
    private static String retornaNumero(String string) {
        return string.replaceAll("[^0-9\\.]", "");
    }
